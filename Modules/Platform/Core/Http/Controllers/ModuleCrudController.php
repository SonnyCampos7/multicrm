...
use Modules\Platform\Notifications\Notifications\GenericNotification;
...
abstract class ModuleCrudController extends AppBaseController
{
    ...
    public function create(Request $request)
    {
        ...
        if ($mode == self::FORM_MODE_SIMPLE) {
            ...
            if ($this->formModalCssClass != null && $request->get('relationType') === null && !$request->get('quickEditModel')) {
    ...
    protected function importProcess(Request $request)
    {
        ...
        if (!empty($data)) {
            ...
            $deduplicateColumn = $request->get('deduplicate_column');
            ...
            foreach ($csvData as $rowKey => $row) {
                ...
                try {
                    ...
                    if ($deduplicateColumn != "") {
                        ...
                        $exist = App::make($this->entityClass)->where($deduplicateColumn, $fields[$deduplicateColumn])->first();

                        if (!$exist) {
                            $entity = $csvImporter->createEntity($fields);
                            $imported++;
                        } else {
                            $entity = $csvImporter->updateEntity($fields, $exist);
                            $updated++;
                        }
            ...
            flash(trans('core::core.import_finished', ['imported' => $imported, 'not_imported' => $notImported, 'updated' => $updated]))->info();
            return redirect()->route($this->routes['index']);
